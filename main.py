...
if __name__ == "__main__":
    ...
    print(translated_string_list)
    for i in range(banyak_individu):
        translated_string = translated_string_list[i]
        translated_string_list[i] = str(translated_string_list[i]).replace('u', "'")
        translated_string_list[i] = str(translated_string_list[i]).replace('l', ",")
    ...
    for i in range(1, banyak_individu+1):
        ...
        content = "\\fixed c' {\n" + "\key " + tangga_nada + " \major" + "\n" +str(translated_string_list[i-1]) + '\n' "}"
        # print(content)
        f = open('test ' + str(i) + '.ly', 'w')
        f.write(title + content)
        f.close()

        content_midi = "\\score {\n" + content + "\n" + "\\midi{ }\n}"
        # print(content_midi)
        f = open('test_midi ' + str(i) + '.ly', 'w')
