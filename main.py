...
komposisi_bass = np.empty((1,anggota_birama))
...
def hitung_fitness_bass(populasi_awal, range_nada, anggota_birama):
    ...
    for i in range(len(populasi_awal)):
        ...
        stabilitas_kontur_list = pfb.stabilitas_kontur(populasi_awal[i], anggota_birama)
        variasi_irama_list = pfb.variasi_irama(populasi_awal[i],anggota_birama,range_nada)
        rentang_irama_list = pfb.rentang_irama(populasi_awal[i],anggota_birama,range_nada)
...
if __name__ == "__main__":
    ...
    for i in range(len(populasi_next_treble)):
        translated.append(tm.translate(populasi_next_treble[i], anggota_birama, range_nada, tangga_nada))
    temp_translated= []
